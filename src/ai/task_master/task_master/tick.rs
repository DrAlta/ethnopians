...
impl<InpulseId: Clone, EntityId: std::hash::Hash + std::fmt::Debug + Clone, Item: Clone>
    TaskMaster<InpulseId, EntityId, Item>
where
    for<'a> Item: TryFrom<&'a str>,
{
    pub fn tick(
        ...
        let Some(sub_system_state) = self.stack.last_mut() else {
            self.tick_counter += 1;
            return TastMasterReport::Err(format!("[{}:{}] no task to preform", file!(), line!()));
        };
        match sub_system_state {
            SubSystemState::BehaviorTree {
                tree_id,
                execution_limb,
                state_tick_next_maybe,
            } => {
                ...
                let Some(tree) = behavoir_tree_tasks.get(tree_id) else {
                    ...
                    match handle_failure(
                        &mut self.stack,
                        format!(
                            "[{}:{}] failed to get behavior tree task: {tree_id2:?}",
                            file!(),
                            line!()
                        ),
                    ) {
                ...
                let exec_report = loop {
                    ...
                    break this_node.down_tick(
                        std::mem::replace(state_tick_next_maybe, None),
                        &mut self.blackboard,
                    );
                };
                return walk_up(
                    ...
                    execution_limb,
                    exec_report,
                    &mut self.prayer_being_waited_on_maybe,
                    /*this_node,*/ path,
                    tree, /*&mut self.blackboard*/
                );
            }
