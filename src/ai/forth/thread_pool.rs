...
impl<InpulseId, EntityId> ThreadPool<InpulseId, EntityId> {
    ...
    pub fn insert(
        &mut self,
        key: ThreadId,
        value: Thread<InpulseId, EntityId>,
    ) -> Option<Thread<InpulseId, EntityId>> {
    ...
    pub fn extend<I: IntoIterator<Item = (ThreadId, Thread<InpulseId, EntityId>)>>(
        &mut self,
        iter: I,
    ) {
        self.0.extend(iter);
...
impl<InpulseId, EntityId, T: Into<BTreeMap<ThreadId, Thread<InpulseId, EntityId>>>> From<T>
    for ThreadPool<InpulseId, EntityId>
{
