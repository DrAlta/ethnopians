        ...
        match self {
            Node::Selector { children } => {
                ...
                if let Some(child_state) = child_state_maybe {
                    logy!(
                        "error",
                        "Excepted a Selector state to not contain child_state, got {child_state:?}"
                    );
                };
            ...
            Node::Sequence { children } => {
                ...
                if let Some(child_state) = child_state_maybe {
                    logy!(
                        "error",
                        "Excepted a Sequence state to not contain child_state, got {child_state:?}"
                    );
                };
                ...
                return Prayer::Status {
                    status: match childs_returned_status {
                        ...
                        Status::Waiting {
                            ...
                            state: State::Inverter {
                                child_state_maybe: Some(Box::new(child_status)),
                            },
                        },
                    },
                }
