...
impl<K: Debug + std::cmp::Eq + std::hash::Hash, V: Debug> Blackboard<K, V> {
    pub fn insert(&mut self, k: K, v: Variable<K, V>) -> Option<Variable<K, V>> {
        ...
        self.stack
        .last_mut().expect("we added an items if the stack was empty")
        .insert(k, v)
    }
        ...
        for i in 1..self.stack.len()+1 {
            if let Some(hashmap) = self.stack.get(self.stack.len() - i) {
                match hashmap.get(current_key) {
                    Some(Variable::Defer(id)) => {
                        current_key = id
                    },
                    Some(Variable::Chit(value)) => {
                        return Some(value)
                    },
                    None => (),
                }
...
mod tests {
    ...
    pub fn defer_some_test() {
        let mut blackboard = Blackboard::from(
            HashMap::from([
                ("a", Variable::Chit(1))
            ])
        );
        blackboard.push(HashMap::from([
            ("b", Variable::Defer("a"))
        ]));
    ...
    pub fn chit_none_test() {
        let blackboard = Blackboard::from(
            HashMap::from([
                ("a", Variable::Chit(1))
            ])
        );
