...
fn step_test() {
    ...
    assert_eq!(cpu.step(&bt), Ok(Status::Running(InpulseId::Act1)));
    assert_eq!(
        &cpu.stack,
        &vec![
            StackItem::Selector(1),
    ...
    assert_eq!(cpu.step(&bt), Ok(Status::Running(InpulseId::Act2)));
    assert_eq!(
        &cpu.stack,
        &vec![
            StackItem::Selector(1),
    ...
    assert_eq!(cpu.step(&bt), Ok(Status::Running(InpulseId::Act3)));
    assert_eq!(
        &cpu.stack,
    ...
    assert_eq!(cpu.step(&bt), Err("program halted".into()));
    assert_eq!(&cpu.stack, &vec![StackItem::Success]);
    assert_eq!(&cpu.return_stack, &Vec::<ExecutionToken>::new());
}
...
fn test() {
    ...
    bt.insert(action1.clone(), Thread::Action(InpulseId::Act1));

    let action2 = "a2".to_owned();
    bt.insert(action2.clone(), Thread::Action(InpulseId::Act2));
    let action3 = "a3".to_owned();
    bt.insert(action3.clone(), Thread::Action(InpulseId::Act3));

    let sequence = "seq".to_owned();
    bt.insert(
        sequence.clone(),
        Thread::Sequence(vec![action1.clone(), action2.clone()]),
    );

    let selector = "sel".to_owned();
    bt.insert(
        selector.clone(),
        Thread::Selector(vec![sequence.clone(), action3.clone()]),
    );
