...
use super::{parse_u8, parse_ident, Thingie};
...
pub fn parse_inventory_have_ge<'a>(
    ...
    Ok((tail, Thingie::Tree(Instruction::InventoryGE(item.to_owned(), number), HashMap::new())))
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn parse_inventory_have_ge_test() {
        let (_, Thingie::Tree(i, _db)) = parse_inventory_have_ge("inventory_have_ge ( stone ,1 )").unwrap() else {
            panic!()
        };
        assert_eq!(
            i,
