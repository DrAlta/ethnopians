...
pub fn collide(
    ...
    let empty = Vec::new();
    let Some(Location::World { x:ax, y: ay }) = world.locations.get(&a) else {
        return empty
    };
    let Some(Location::World { x:bx, y: by }) = world.locations.get(&b) else {
        return empty
    };

    let Some((aw, ah )) = world.sizes.get(&a) else {
        return empty
    };
    let Some((bw, bh )) = world.sizes.get(&b) else {
        return empty
    };
    ...
    let new_x = if x_overhang > 0.0 {
        bx - aw
    } else {
        bx + bw
    };
    let new_y = if y_overhang > 0.0 {
        by - ah
    } else {
        by + bh
    };
