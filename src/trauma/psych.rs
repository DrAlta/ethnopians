...
impl Psych {
    ...
    fn get_ability_to_deal_with_stress_while_dysregulated(
        &self,
    ) -> (Number, Number) {
       (
    ...
    fn get_ability_to_handle_emotional_experience_type(
        ...
        self.recent_experiences.iter().filter_map(
            |(_id, experience)|
            {
                if &experience.trauma_type == trauma_type{
                    Some((
                        self.dysregulated_gradient,
                        self.dysregulated_y_intercept
                    ))
                } else {
                    None
                }
    ...
    pub fn calc_trauma_for_bad_experince(&self, base_traumatic_stressfullness: Number) -> Number {
        let (gradient, y_intercept) =
            self.get_ability_to_deal_with_stress_while_dysregulated();
        ...
        match event {
            ...
            Event::Trauma { trauma_type, base_traumatic_stressfulness } => {
                ...
                let (new_gradient_delta, new_y_intercept_delta) = if bad_experience_ka {
                    ...
                    let new_y_intercept_delta = (y_intercept * (x / trauma_tolerance)) - y_intercept;
                    ...
                    let new_gradient_delta = - (gradient * base_gradient_improvement_percent * comfort_zone_factor);
                    ...
                    let new_y_intercept_delta = -(y_intercept * perscent_over_tolerance * good_experience_factor);
                    (new_gradient_delta, new_y_intercept_delta)
                };
                // add the event to the experince
                match self.get_trauma_experience_mut(&trauma_experince_id) {
                    Some(experience) => experience.add_event(new_gradient_delta, new_y_intercept_delta, trauma, base_traumatic_stressfulness),
                    None => {
                        let experience = TraumaExperience::simple_new(new_gradient_delta, new_y_intercept_delta, trauma, base_traumatic_stressfulness, trauma_type);
