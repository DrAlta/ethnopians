...
pub struct Psych {
    ...
    ability_to_handle_emotional_experience_type:
        BTreeMap<TraumaExperienceType, (Fraction, Fraction)>,
}
...
impl Psych {
    ...
    pub fn handle_experience_stage_1(&mut self, event: Event) -> Stage1EmotionalExperience {
        ...
        self.num_experiences
            .add_or_insert(experience_type.clone(), 1);
    ...
    pub fn handle_experience_stage_2(
        ...
        if bad_experience_ka {
            ...
            let comfort_zone_factor = Fraction::from(1)
                / (Fraction::from(1)
                    + (Fraction::try_new(1, 2).unwrap() * self.location_in_comfort_zone.clone()));
            let (gradient, y_intercept) =
                self.get_ability_to_handle_emotional_experience_type(&experience_type);
            ...
            let base_gradient_improvement_percent = traumatic_stress.clone()
                / (traumatic_stress.clone() + Fraction::try_new(4, 1).unwrap());
            ...
            let good_experience_factor = Fraction::try_from(
                ((self.get_num_good_experiences(&experience_type) + 1) as f64).sqrt(),
            )
            .unwrap();
            let perscent_over_threshold =
                (traumatic_stress.clone() / trauma_threshold) - Fraction::from(1);
        ...
        self.recent_experiences.push(TraumaExperience::simple_new(
            experience_type,
            trauma,
            traumatic_stress,
        ))
