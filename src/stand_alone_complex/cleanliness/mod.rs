...
fn calculate_total_effective_space(
    ...
    let total_saved_space: f32 = compatibility
        .iter()
        .filter_map(|(pair_types, compatibility_value)| {
            // Check if both types in the pair exist in the current mixture
            let amounts: Vec<&f32> = pair_types.iter().filter_map(|t| mixture.get(t)).collect();
