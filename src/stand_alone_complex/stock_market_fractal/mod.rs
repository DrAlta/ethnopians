...
impl<const N: usize, I: Iterator<Item = Number>> Fractal<N, I> {
    pub fn sample<RNG: RandomNumberGenerator + RandoRange<Number>>(
        &mut self,
        rng: &mut RNG,
    ) -> Option<Number> {
        let now = *self.last.iter().max().unwrap();
        let start = now + 1;
        let next: [usize; N] = std::array::from_fn(|idx| {
            start
                + match self.turns.find_first(&idx) {
                    Some(x) => x,
                    None => {
                        fix::<N, RNG>(&mut self.turns, rng);
                        self.turns
                            .find_first(&idx)
                            .expect("we added it if it was missing")
                    }
                }
        });
        let now1 = now as f32;
        ...
        for idx in 0..N {
            ...
            ret += match layer.run_idx % 5 {
                0 => (&layer.s) + (&layer.h1 - &layer.s) * t,
                1 => (&layer.h1) + (&layer.l1 - &layer.h1) * t,
                2 => (&layer.l1) + (&layer.h2 - &layer.l1) * t,
                3 => (&layer.h2) + (&layer.l2 - &layer.h2) * t,
                4 => {
...
pub fn foo<
    ...
    RNG: RandomNumberGenerator + RandoRange<Number>,
>(
    mut samples: [I; N],
    rng: &mut RNG,
) -> Option<Fractal<N, I>> {
    ...
    Some(Fractal {
        samples,
        layer_states: layers_runs.try_into().unwrap(),
        turns,
        last,
    })
}
...
fn fix<const N: usize, RNG: RandomNumberGenerator>(selfie: &mut Vec<usize>, rng: &mut RNG) {
    ...
    needed.retain(|x| !selfie.contains(x));
    while !needed.is_empty() {
        let dice = rng.random_range(0, (N * 2) + needed.len());
        if dice < N {
            selfie.push(dice);
            needed.retain(|x| x != &dice);
        } else {
