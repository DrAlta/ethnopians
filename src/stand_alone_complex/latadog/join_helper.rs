use crate::stand_alone_complex::latadog::{GetKey, Relation, gallop};
    ...
    while !slice1.is_empty() && !slice2.is_empty() {
        ...
        match input1 .get(0).cmp(input2.get(0)) {
            ...
            Ordering::Equal => {
                ...
                let count1 = slice1.iter().take_while(|&&x| input1.get(x) == input1.get(0)).count();
                let count2 = slice2.iter().take_while(|&&x| input2.get(x) == input2.get(0)).count();

                println!("c1:{count1}\nc2:{count2}");
                // Produce results from the cross-product of matches.
                for index1 in 0 .. count1 {
                    for index2 in 0 .. count2 {
                        result(input1.get(slice1[index1]), &input1.elements[slice1[index1]], &input2.elements[slice2[index2]]);
                    }
                }
