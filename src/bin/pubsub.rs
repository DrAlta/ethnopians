...
fn main() {
    ...
    println!("{:?}", join([vec![("A".to_owned(), "B".to_owned(),)]], &a));
}
        ...
        match datum_type {
            DatumType::I8 => {
                ...
                if &a1.a_title == first_field {
                    working = Foo::<0>::get_i8_iter(&a1.tuples)
                        .enumerate()
                        .map(|(k, v)| (k, v.clone()))
                        .collect();
                } else {
                    working = Foo::<1>::get_i8_iter(&a1.tuples)
                        .enumerate()
                        .map(|(k, v)| (k, v.clone()))
                        .collect();
                }
...
fn look_tables_type<T: Foo<0> + Foo<1> + GetDatumType<0> + GetDatumType<1>>(
    table_id: &String,
    field_id: &String,
    db: &HashMap<String, Relation<T>>,
) -> Option<DatumType> {
